...
This dashboard loads data from a server via XHR requests and is able to visualize data in under 1 second.
\bigskip
...
The dashboard is able to read saved report output from IBM Analytics (aka IBM Cognos). Which comes in handy as this dashboard uses the IBM Cognos REST API to just fetch the data and avoids to load the IBM Cognos Portal files reducing overhead to a minimum by focusing on speed and performance to visualize the raw data. Fetching data from REST APIs is of course not limited to IBM Cognos. We would be happy to know from which services or servers you load the dashboard data. See support chapter on how to contact us.
...
The src-folder contains the project files that must be compiled. The compiled files reside in dist-folder.
We provide a docker-compose.yml file for you to start everything needed inside a container.
